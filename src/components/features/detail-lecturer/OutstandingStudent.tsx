...
const StudentCard = ({
  ...
  <div
    className={cn(
      'group relative mx-auto h-full w-[344px] overflow-hidden rounded-lg border bg-white shadow-md transition-all duration-300',
      isPageCourse ? 'hover:bg-[#083AD4]' : 'hover:bg-[#E6F0FF]'
    )}
  >
    <div className="h-full select-none md:flex md:flex-col md:items-center md:justify-between">
      ...
      <div className="flex-1 px-5 pt-6 md:px-8 md:pt-9">
        <div className="h-[294px] w-full overflow-hidden md:h-[297px] md:w-[282px]">
          <img
            src={item.avatar_url || icons.defaultAvatar}
            alt={item.name}
            className={cn(
              'h-full w-full object-cover transition-transform duration-300',
              isPageCourse && 'group-hover:scale-110'
            )}
          />
        </div>
      </div>
      <div
        className={cn(
          'relative flex w-full flex-col gap-2 bg-[#083AD4] transition-colors duration-300 px-4 py-5 h-full',
          !isPageCourse && 'group-hover:bg-[#3464F8]'
        )}
      >
