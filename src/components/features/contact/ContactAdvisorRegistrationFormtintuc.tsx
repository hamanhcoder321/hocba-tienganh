...
const levelOptions = [
  'Mất gốc/ mới bắt đầu',
  'Đã học cơ bản (tầm IELTS 4.0-5.0)',
  'Đã học trung cấp (tầm IELTS 5.5-6.5)',
  'Đã có nền, cần ôn thi / dùng cho công việc',
];

const contactOptions = ['Gọi điện trực tiếp', 'Nhắn Zalo', 'Email'];

const timeOptions = ['Sáng (8h-12h)', 'Chiều (13h30-18h)', 'Tối (Sau 18h)'];

const courseOptions = [
  'Khóa Khởi Động (0-3.0+)',
];
...
export default function ContactAdvisorRegistrationForm({
  ...
  const { form, isPending, isSuccess, handleRegisterStudy } = useRegisterStudy({
    wishlist_courses: 'Khóa Khởi Động (0-3.0+)'
  });
  ...
  return (
    <div className={cn('relative mx-auto mt-0 w-full max-w-[500px] md:-mt-5', className)}>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 md:space-y-5">
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2 md:gap-x-6 md:gap-y-3">
            ...
            {/* 4 field nâng cao */}
              ...
              <div className="space-y-2 md:col-span-1">
                <Label htmlFor="time" className="text-[13px] font-semibold text-white md:text-base whitespace-nowrap">
          ...
          {isCourseChinese ? (
            ...
            <div className="relative mx-auto w-full pt-4 text-center">
              <Button
                type="submit"
                disabled={isPending}
                className="relative mx-auto flex items-center justify-center gap-[10px] min-h-[50px] w-[85%] max-w-[320px] translate-x-2 md:translate-x-4 rounded-[24px] border-[3px] border-white bg-[#F97316] py-[6px] px-[16px] text-base font-black uppercase text-white shadow-lg transition-colors hover:opacity-90 disabled:opacity-70 md:w-full md:max-w-[386px] md:min-h-[68px] md:text-2xl"
              >
              ...
              <p className="mt-2 text-center text-[12.5px] font-normal text-white md:text-right md:text-[15px] md:leading-[150%]">
                THE IELTS SPACE sẽ liên hệ lại trong vòng <span className="font-bold">24 giờ</span> làm việc.
              </p>
            </div>
          )}

          {/* slot cho content bổ sung — SimpleFormRegister nhét ảnh mascot mobile vào đây */}
          {children}
        </form>
      </div>
    </div>
