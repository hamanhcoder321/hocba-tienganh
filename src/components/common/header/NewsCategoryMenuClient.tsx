...
async function loadNewsCategories(): Promise<MenuItem[]> {
  try {
    ...
    return categoryLists
      .filter((category: BlogCategory) => NEWS_SLUGS.includes(category.slug || ''))
      .map((category: BlogCategory) => ({
        title: category.name,
        href: category.slug || '',
      }));
  } catch (error) {
    console.error('Error fetching news categories for header menu (client):', error);
    return [];
  }
}

export const NewsCategoryMenuDesktop: React.FC = () => {
  const [items, setItems] = useState<MenuItem[]>([]);

  useEffect(() => {
    void loadNewsCategories().then(setItems);
  }, []);
  ...
  return (
    <div
      ...
      role="menu"
      aria-label="Menu Tin tức & sự kiện"
    >
...
export const NewsCategoryMenuMobile: React.FC = () => {
  const [items, setItems] = useState<MenuItem[]>([]);
